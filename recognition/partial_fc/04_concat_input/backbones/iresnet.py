...
class IBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.conv2 = conv3x3(planes, planes, stride, groups=2)
...
class IResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_features=512, zero_init_residual=False,
        ...
        self.conv1 = nn.Conv2d(6, self.inplanes, kernel_size=3, stride=1, padding=1, groups=2,
                               bias=False)
        ...
        self.fc = nn.Conv2d(512 * block.expansion, num_features, kernel_size=7, groups=2)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                conv1x1(self.inplanes, planes * block.expansion, stride, groups=2),
    ...
    def forward(self, x):
        h, w = x.shape[-2:]
        rois = get_rois(x)
        tp = roi_tanh_polar_warp(x, rois, (h,w), keep_aspect_ratio=True).clip(-1, 1)
